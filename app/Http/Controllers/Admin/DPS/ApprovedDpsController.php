...
class ApprovedDpsController extends Controller
{
    /**
    ...
    {
        return view('admin.dps.approved-dps.index');
    }
    ...
    {
        if ($request->ajax()) {
            $model = DpsAccount::where('approval', 'Approved')->get();
            return Datatables::of($model)
                ...
                ->addColumn('action', function ($model) {
                    return view('admin.dps.approved-dps.action', compact('model'));
                })
    ...
    public function show($uuid)
    {
        ...
        $deposit_info = Transaction::where('dps_account_id', $account_information->id)->where('tx_type', 'dps payment')->orderBy('id', 'DESC')->get();

        $withdraw_info = Transaction::where('dps_account_id', $account_information->id)->where('tx_type', 'dps repay')->orderBy('id', 'DESC')->get();

        // $dps_acc_info = $account_information;

        $payment_status = current_payment_status('dps withdraw', $account_information->id);

        return view('admin.dps.approved-dps.show', compact('account_information', 'deposit_info', 'withdraw_info', 'payment_status'));
    }
    ...
    public function edit($uuid)
    {
        ...
        return view('admin.dps.approved-dps.edit', compact('model', "members", 'dps_types'));
    }
    ...
    public function update(Request $request, $uuid)
    {
        ...
        // Now Updating The Transaction Table 
    ...
    {
        $account_information = DpsAccount::where('uuid', $uuid)->firstOrFail();
        return view('admin.dps.approved-dps.approve', compact('account_information'));
    }
