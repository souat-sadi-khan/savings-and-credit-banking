...
<div class="tile">
    <div class="tile-body">
        <div class="col-md-12" style="margin-top:-10px">
            <hr>
            <h4 class="text-center text-info">Sundry Of {{$sundry_type}} Calculation For: <span class="text-danger">{{$sundry_month}}</span>
            </h4>
            <hr>
        </div>
        <div class="col-md-12">
  ...
  <tbody class="bg-info">
      ...
      @for ($i=0; $i < $no_of_accounts; $i++)
          @php
              ...
              $account_no = get_id_account_no('share',$sundry_info['info'][$i]->prefix_share,$sundry_info['info'][$i]->code_share);
              $member_id = get_id_account_no('member',$sundry_info['info'][$i]->prefix,$sundry_info['info'][$i]->code).', '.$sundry_info['info'][$i]->name_in_bangla.', '.$sundry_info['info'][$i]->contact_number;
      ...
      <tr >
        ...
        <td><input type="text" readonly name="member[]" id="member" class="form-control" placeholder="Member ID"  value="{{ $member_id }}"></td>
      ...
      <tr>
   <td colspan="3" class="text-right text-light"><h4 >Total Sundry Amount Of Share</h4></td>
   <td class="text-light"><h4>{{ $sundry_info['total'] }}</h4></td>
</tr>
      @else
    <tr>
        <td colspan="4" class="text-center text-light"><h4 >No Share Account Fount</h4></td>
      </tr>
      @endif
