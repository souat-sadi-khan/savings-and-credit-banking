...
<div class="tile">
    <div class="tile-body">
        <div class="col-md-12" style="margin-top:-10px">
            <hr>
            <h4 class="text-center text-info">Sundry Of Double Benefit Calculation For: <span class="text-danger">{{$sundry_month}}</span>
            </h4>
            <hr>
        </div>
        <div class="col-md-12">
             <form action="{{route('admin.sundry-calculation.store')}}" method="post" id="content_form">
            @csrf
  ...
  <tbody class="bg-info">
      ...
      @for ($i=0; $i < $no_of_accounts; $i++)
          @php
              ...
              $account_no = get_id_account_no('double_benifit',$sundry_info['info'][$i]->prefix,$sundry_info['info'][$i]->code);
              $member_ac = get_id_account_no('member',$sundry_info['info'][$i]->member->prefix,$sundry_info['info'][$i]->member->code).', '.$sundry_info['info'][$i]->member->name_in_bangla.', '.$sundry_info['info'][$i]->member->contact_number;
      ...
      <tr >
        ...
        <input type="hidden" readonly name="member_id[]" id="member_id" class="form-control"  value="{{ $member_id }}">
      ...
      @endfor
         <tr>
        <td colspan="3" class="text-right text-light"><h4 >Total Sundry Amount Of Double Benifit</h4></td>
        <td class="text-light"><h4>{{ $sundry_info['total'] }}</h4></td>
    </tr>
    @else 
    <tr>
        <td colspan="4" class="text-center text-light"><h4 >No Double Benefit Account Found</h4></td>
      </tr>
      @endif
