 ...
 <div class="tab-content" id="myTabContent">
     ...
     {{-- ::::::::::    Loan from member Info Tab Content    ::::::::: --}}
     <div class="tab-pane fade show active" id="double-benifit-info" role="tabpanel" aria-labelledby="double-benifit-info-tab">
           <div class="row" style="margin-bottom:25px;margin-top:25px">
             <div class="col-md-6">
                 ...
                 <span>Duration: {{ $fdr_info->loan_duration }} {{ $fdr_info->loan_duration_type }}</span><br>
                 <span>Issue: {{ carbonDate( $fdr_info->issue_date) }}</span><br>
             ...
             <tbody>
                 ...
                 @foreach ($member_informations as $member_information)
                     <tr class="bg-success text-light text-center">
                         <td colspan="2">Member {{ ++$i }}</td>
                     </tr>
                      <tr>
                     <td>Name</td>
                     <td>{{$member_information->member->name_in_bangla}}</td>
                 </tr>

                 <tr>
                     <td>ID</td>
                     <td>
                         {{$member_information->member->prefix}}
                         {{numer_padding($member_information->member->code, get_option('digits_member_code'))}}
                    </td>
                 </tr>

                 <tr>
                     <td>Father</td>
                     <td>{{$member_information->member->father_name}}</td>
                 </tr>

                 @if ($member_information->member->husband_name)
                    <tr>
                        <td>Mother</td>
                        <td>{{$member_information->member->mother_name}}</td>
                    </tr>
                 @endif

                 <tr>
                     <td>Mobile</td>
                     <td>{{$member_information->member->contact_number}}</td>
                 </tr>

                 <tr>
                    <td>Address</td>
                    <td>
                        {{$member_information->member->present_address_line_1}},
                        {{$member_information->member->present_address_line_1}}, {{$member_information->member->present_city}},
                        {{$member_information->member->present_zipcode}}
                    </td>
                 ...
                 <tr>
                     <td>Image</td>
                     <td>
                         <a href="{{asset('storage/member/'.$member_information->member->photo)}}" target="_blank">
                             <img src="{{asset('storage/member/'.$member_information->member->photo)}}" width="30%"
                 ...
                 <tr>
                     <td>Signature</td>
                     <td>
                         <a href="{{asset('storage/member/'.$member_information->member->signature)}}" target="_blank">
                             <img src="{{asset('storage/member/'.$member_information->member->signature)}}" width="30%"
                                 alt="Image Not Uploaded." class="rounded img-thumbnail">
                         </a>
                     </td>
                 </tr>
