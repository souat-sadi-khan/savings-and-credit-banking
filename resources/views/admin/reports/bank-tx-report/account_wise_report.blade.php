@extends('layouts.app', ['title' => _lang('Account Wise Report'), 'modal' => 'lg'])
...
<div class="app-title">
    <div>
        <h1 data-placement="bottom" title="Account Wise Report."><i class="fa fa-universal-access mr-4"></i>
            {{_lang('Account Wise Report')}}</h1>
        <p>{{_lang('Account Wise Report. Here you can See Account Wise Report')}}</p>
    </div>
                ...
                <div id="print_table" style="color:black">
                    ...
                    <div class="text-center col-md-12">
                        <h4 style="margin:0px ; margin-top: 7px; border:solid 1px #000; border-radius:50px; display:inline-block; padding:10px;"
                            class="{{ $report_type =='BANK ACCOUNT WISE DEPOSIT REPORT'?"bg-success ":"bg-danger" }} text-light">
                            <b>{{$report_type}}</b></h4>
                    </div>
                    ...
                    <div class="col-md-12 text-left mb-3">
                        <p style="margin:0px ;"> Account No : <span class="ml-1">{{$account_info->account_no}}</span> </p>
                            <p style="margin:0px ;"> Bank : <span class="ml-1">{{$account_info->bank_name}}</span> </p>
                            <p style="margin:0px ;"> Branch : <span class="ml-1">{{$account_info->branch_name}}</span> </p>
                            <p style="margin:0px ;"> AC Holder : <span class="ml-1">{{$account_info->account_holder_name}}</span> </p>
                    </div>
                        ...
                        <table class="table table-bordered ">
                            <thead class="table-head-bg">
                                ...
                                <tr>
                                    ...
                                    <th scope="col">{{ $report_type =='BANK ACCOUNT WISE DEPOSIT REPORT'?"Deposit ":"Withdraw"  }}
                                        Amount</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @foreach ($tx_info as $tx)
                                @php
                                $i++;
                                @endphp
                                <tr>
                                    <td>{{$i}}</td>
                                    <td>{{ carbonDate($tx->tx_date)}}</td>
                                    <td>{{ $tx->grand_total_amt }}</td>
                                </tr>
                                @endforeach
                                <tr class="text-weight-bold">
                                    <td colspan="2" class="text-right">Total
                                        {{ $report_type =='BANK ACCOUNT WISE DEPOSIT REPORT'?"Deposit ":"Withdraw"  }}</td>
                                    <td>{{$total}}</td>
                                </tr>
                                @else
                                <tr>
