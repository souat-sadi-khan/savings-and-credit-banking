@extends('layouts.app', ['title' => _lang(($report_type =="BANK DEPOSIT GENERAL REPORT"?"Deposit ":"Withdraw"). 'General
Report'), 'modal' => 'lg'])
...
<div class="app-title">
    <div>
        <h1 data-placement="bottom" title='{{ $report_type =="BANK DEPOSIT GENERAL REPORT"?"Deposit ":"Withdraw" }} Report.'><i
                class="fa fa-universal-access mr-4"></i>
            {{_lang(' Report')}}</h1>
        <p>{{_lang(($report_type =="BANK DEPOSIT GENERAL REPORT"?"Deposit ":"Withdraw").' Report. Here you can See '.($report_type =="BANK DEPOSIT GENERAL REPORT"?"Deposit ":"Withdraw").'  Report')}}
        </p>
    </div>
                ...
                <div id="print_table" style="color:black">
                    ...
                    <div class="text-center col-md-12">
                        <h4 style="margin:0px ; margin-top: 7px; border:solid 1px #000; border-radius:50px; display:inline-block; padding:10px;"
                           class="{{ $report_type =="BANK DEPOSIT GENERAL REPORT"?"bg-success ":"bg-danger" }} text-light">
                            <b>{{$report_type}}</b></h4>
                    </div>
                        ...
                        <table class="table table-bordered ">
                            <thead class="table-head-bg">
                                ...
                                <tr>
                                    ...
                                    <th scope="col">{{ $report_type =="BANK DEPOSIT GENERAL REPORT"?"Deposite ":"Withdraw"  }}
                                        Amount</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @php
                                $i++;
                                $acc_info = App\models\bank_accounts\BankAccount::find($key);
                                @endphp
                                <tr>
                                    <td>{{$i}}</td>
                                    <td>{{ $acc_info->account_no}}</td>
                                    <td>{{ $acc_info->bank_name}}</td>
                                    <td>{{ $acc_info->branch_name}}</td>
                                    <td>{{ $acc_info->account_holder_name}}</td>
                                    <td>{{ $value }}</td>
                                </tr>
                                @endforeach
                                <tr class="text-weight-bold">
                                    <td colspan="5" class="text-right">Total
                                        {{ $report_type =="BANK DEPOSIT GENERAL REPORT"?"Deposit ":"Withdraw"  }}</td>
                                    <td>{{$total}}</td>
                                </tr>
                                @else
                                <tr>
                                    <td colspan="6" align="center" class="text-danger">No
                                        {{ $report_type =="BANK DEPOSIT GENERAL REPORT"?"Deposit ":"Withdraw"  }} Found</td>
                                </tr>
                                @endif
                            </tbody>
                        </table>
