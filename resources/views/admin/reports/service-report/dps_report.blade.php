@extends('layouts.app', ['title' => _lang('DPS Report'), 'modal' => 'lg'])
...
<div class="app-title">
    <div>
        <h1 data-placement="bottom" title="DPD Report."><i class="fa fa-universal-access mr-4"></i>
            {{_lang('DPS Report')}}</h1>
        <p>{{_lang('DPS Report. Here you can See DPS Report')}}</p>
    </div>
                ...
                <div id="print_table" style="color:black">
                    ...
                    <div class="text-center col-md-12">
                        <h4 style="margin:0px ; margin-top: 7px; border:solid 1px #000; border-radius:50px; display:inline-block; padding:10px;"
                            class="{{ ($dps_withdraw_confirmation == false && $dps_deposit_confirmation == true) ?'bg-danger':'bg-success'}} text-light">
                            <b>{{$report_type}}</b></h4>
                    </div>
                    <br>
                    ...
                    <div class="table-responsive border-dark">
                        <table class="table table-bordered ">
                            <thead class="table-head-bg">
                                @if ($report_type == 'DPS DEPOSIT & WITHDRAW')
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{++$i}}</td>
                                    {{-- {{ dd($inf->dps_ac) }} --}}
                                    <td>{{get_id_account_no('dps',  $inf->dps_ac?$inf->dps_ac->prefix:'',  $inf->dps_ac?$inf->dps_ac->code:'')}}
                                    </td>
                                ...
                                @for ($i = 0; $i < $max_row; $i++) <tr>
                                    @if($i<$no_of_deposit) <td>
                                        {{get_id_account_no('dps',  $dps_deposited[$i]->dps_ac?$dps_deposited[$i]->dps_ac->prefix:'',  $dps_deposited[$i]->dps_ac?$dps_deposited[$i]->dps_ac->code:'')}}
                                        </td>

                                        <td>{{get_id_account_no('member', $dps_deposited[$i]->member?$dps_deposited[$i]->member->prefix:'', $dps_deposited[$i]->member?$dps_deposited[$i]->member->code:'')}}
                                        </td>
                                        <td>{{$dps_deposited[$i]->member?$dps_deposited[$i]->member->name_in_bangla:''}}  </td>
                                        <td>{{carbonDate($dps_deposited[$i]->tx_date)}}</td>
                                        <td>{{number_format ( $dps_deposited[$i]->grand_total_amt,2)}}</td>
                                        @else
                                        <td></td>
                                        <td></td>
                                        <td></td>
                                        ...
                                        @if($i<$no_of_withdraw) <td>
                                            {{get_id_account_no('dps',  $dps_withdrawed[$i]->dps_ac?$dps_withdrawed[$i]->dps_ac->prefix:'',  $dps_withdrawed[$i]->dps_ac?$dps_withdrawed[$i]->dps_ac->code:'')}}
                                            </td>
                                            <td>{{get_id_account_no('member', $dps_withdrawed[$i]->member?$dps_withdrawed[$i]->member->prefix:'', $dps_withdrawed[$i]->member?$dps_withdrawed[$i]->member->code:'')}}
                                            </td>
                                            <td>{{$dps_withdrawed[$i]->member?$dps_withdrawed[$i]->member->name_in_bangla:''}}  </td>
                                            <td>{{carbonDate($dps_withdrawed[$i]->tx_date)}}</td>
                                            <td>{{number_format ( $dps_withdrawed[$i]->grand_total_amt,2)}}</td>
                                            @else
                                            <td></td>
                                            <td></td>
                                            <td></td>
