    ...
    var _componentRemoteModalLoad = function () {
        ...
        $(document).on('change', '#member_id', function () {
            ...
            $.ajax({
                ...
                .done(function (data) {
                    $("#dps_account_id").html(data.options);
                })
        });
        ...
        $(document).on('change', '#dps_account_id', function () {
            var dps_account_id = $(this).val();
            var url = $(this).data('url');
            $("#total_savings").val('');
            $("#grand_total_savings").val('');
            $("#savings_in_hand").val('');
            ...
            $.ajax({
                    ...
                    type: 'post',
                    data: {
                        dps_account_id: dps_account_id
                    },
                    dataType: 'json'
                })
                .done(function (data) {

                    var grand_total_dps = data.payment_status.currntly_in_hand + data.dps_acc_info.per_month_dps_amt;


                    if (parseInt(grand_total_dps) > parseInt(data.dps_acc_info.total_dps_amt)) {
                        swal("Reminder !", "DPS Already Paid.", "error");
                        ...
                        $("#new_deposit").val(data.dps_acc_info.per_month_dps_amt);
                        $("#total_dps").val(data.payment_status.currntly_in_hand);
                        $("#grand_total_dps").val(grand_total_dps);
                        $("#previous_payment_records").html(data.previous_payment_records);
                        // dps no calculation

                        var dps_duration = data.dps_acc_info.dps_duration;
                        var dps_duration_type = data.dps_acc_info.dps_duration_type;

                        if (dps_duration_type == "Year") {
                            var no_of_dps = dps_duration * 12;
                        } else {
                            var no_of_dps = dps_duration;
                        }

                        var paid_dps = data.payment_status.deposit_times;
                        var due_dps = no_of_dps - paid_dps;

                        $("#no_of_dps").val(no_of_dps);
                        $("#paid_dps").val(paid_dps);
                        $("#due_dps").val(due_dps);
                    }
