...
$(document).ready(function () {
    ...
    $(document).on('change click', '#member_id', function () {
    ...
    $(document).on('change', '#loan_type', function () {
        ...
        var loan_amt = $("#double_benifit_amount").val();
        $("#installment_no").val("");
    ...
    $(document).on('keyup blur change', '#loan_amt', function () {
        installment_calc();
    ...
    $(document).on('keyup blur change', '#loan_type', function () {
        installment_calc();
    });
...
function installment_calc() {
    var loan_amt = check_null_value($("#loan_amt").val());
    var interest_rate = check_null_value($("#interest_rate").val());

    total_interest_amt = loan_amt * interest_rate / 100;
    grand_total_amt = loan_amt * 1 + total_interest_amt * 1;
    ...
    if ($("#round").prop("checked") == true) {
        ...
        grand_total_amt = Math.round(grand_total_amt);
    } else {

        total_interest_amt = total_interest_amt.toFixed(2);
        grand_total_amt = grand_total_amt.toFixed(2);
    }

    $("#total_interest_amt").val(total_interest_amt);
    $("#grand_total_amt").val(grand_total_amt);
